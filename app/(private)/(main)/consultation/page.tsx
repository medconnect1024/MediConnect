...
const PatientCard: React.FC<PatientCardProps> = ({
  ...
  if (patientInfo === null || "error" in patientInfo) {
    return <div className="text-sm text-red-500">{patientInfo?.error || "Patient not found"}</div>
  }
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={onOpenChange}>
        ...
        >
          <CardContent className="p-2">
            <div className="flex justify-between items-center">
              <p className={`font-medium text-sm ${isSelected ? "text-white" : ""}`}>
                {firstName}
              </p>
              <div className="flex items-center">
                <p className={`text-xs mr-2 ${isSelected ? "text-white" : "text-muted-foreground"}`}>
                  {gender}, {age} years
                </p>
      ...
      <CollapsibleContent className="px-2">
        <div className={`p-2 rounded-b-md ${isSelected ? "bg-gray-100" : "bg-gray-100"}`}>
          <p className="text-sm">
          ...
          <div className="mt-2 flex justify-between items-center">
            <p className="text-sm">
              ...
              <span className={appointmentStatus === "attending" ? "text-blue-500" : appointmentStatus === "completed" ? "text-green-500" : "text-yellow-500"}>
                {appointmentStatus}
              </span>
            </p>
...
const Sidebar: React.FC<{
...
}> = ({ onPatientSelect, selectedPatientId, openPatientId, setOpenPatientId }) => {
  const { user } = useUser()
  const doctorId = user?.id

  const appointments = useQuery(api.patients.getAppointmentsByDoctor, doctorId ? { doctorId } : "skip")
  const updateAppointmentStatus = useMutation(api.patients.updateAppointmentStatus)
  ...
  if (!doctorId) {
    return <p className="text-center mt-4">Doctor information not available.</p>
  }
  ...
  if (appointments === null) {
    return <p className="text-center mt-4">Error loading appointments. Please try again.</p>
  }

  const onQueuePatients = appointments.filter((appointment) => appointment.status === "Scheduled")
  const attendingPatients = appointments.filter((appointment) => appointment.status === "attending")
  const completedAppointments = appointments.filter((appointment) => appointment.status === "completed")

  const totalPatients = onQueuePatients.length + attendingPatients.length + completedAppointments.length
  ...
  return (
    <aside className="w-full md:w-64 overflow-y-auto border-r p-4 flex flex-col h-full">
          ...
          {onQueuePatients.length > 0 ? (
            onQueuePatients.map((appointment) => (
              <PatientCard
                ...
                onOpenChange={(open) => setOpenPatientId(open ? Number(appointment.patientId) : null)}
      ...
      <div className="mb-6 flex-grow">
        <h3 className="text-sm font-medium mb-2">Attending ({attendingPatients.length})</h3>
        <div>
          {attendingPatients.length > 0 ? (
            ...
            <p className="text-sm text-gray-500">No patients currently being attended.</p>
          )}
        </div>
      </div>
