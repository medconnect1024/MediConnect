...
import { CoffeeIcon, LayoutDashboardIcon, SettingsIcon, ThermometerIcon, DropletIcon, AlertCircleIcon, CalendarIcon, TruckIcon, DollarSignIcon, PhoneIcon, MailIcon, PlusCircleIcon, UserPlusIcon, UsersIcon } from 'lucide-react'
...
export default function TeaVendingDashboard() {
  ...
  const vendingMachines: VendingMachine[] = [
    { 
      ...
      deliveryBoy: { name: 'John Doe', location: 'En route', eta: '15 minutes' },
  ...
  const selectedMachineData = selectedMachine === 'all' ? null : vendingMachines.find(vm => vm.id === selectedMachine)

  const toggleMachineStatus = (machineId: string) => {
    console.log(`Toggling ${machineId} ${vendingMachines.find(vm => vm.id === machineId)?.status === 'online' ? 'offline' : 'online'}`)
  }

  const transactions: Transaction[] = [
    { id: 'T001', time: '09:15 AM', product: 'Green Tea', category: 'green', amount: 3.50 },
    { id: 'T002', time: '10:30 AM', product: 'Black Tea', category: 'black', amount: 3.00 },
    { id: 'T003', time: '11:45 AM', product: 'Oolong Tea', category: 'oolong', amount: 4.00 },
  ]

  const totalSales = transactions.reduce((sum, t) => sum + t.amount, 0)
  const categorySales = transactions.reduce((acc: { [key: string]: number }, t) => {
    acc[t.category] = (acc[t.category] || 0) + t.amount
    return acc
  }, {})

  const handleMachineInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  const handleMachineSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const accessToken = Math.random().toString(36).substr(2, 9) + Date.now().toString(36)
    console.log('New machine onboarded:', { ...newMachine, accessToken })
    toast.success(`${newMachine.name} has been successfully added with ID: ${newMachine.id}`)
  ...
  const handleVendorSubmit = (e: React.FormEvent) => {
    ...
    toast.success(`${newVendor.name} from ${newVendor.company} has been successfully added.`)
      ...
      <header className="bg-white dark:bg-gray-800 shadow-md p-4">
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center">
          <div className="flex items-center space-x-4 mb-4 md:mb-0">
            ...
            <h1 className="text-3xl font-bold text-emerald-800 dark:text-emerald-200">TeaVend Dashboard</h1>
          </div>
            ...
            <Dialog open={isAddMachineOpen} onOpenChange={setIsAddMachineOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>Enter the details for the new vending machine.</DialogDescription>
                </DialogHeader>
                <form onSubmit={handleMachineSubmit}>
                  <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="id" className="text-right">ID</Label>
                      <Input id="id" name="id" value={newMachine.id} onChange={handleMachineInputChange} className="col-span-3" required />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="name" className="text-right">Name</Label>
                      <Input id="name" name="name" value={newMachine.name} onChange={handleMachineInputChange} className="col-span-3" required />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="location" className="text-right">Location</Label>
                      <Input id="location" name="location" value={newMachine.location} onChange={handleMachineInputChange} className="col-span-3" required />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="description" className="text-right">Description</Label>
                      <Textarea id="description" name="description" value={newMachine.description} onChange={handleMachineInputChange} className="col-span-3" />
                    </div>
                  </div>
                  <DialogFooter>
            ...
            <Dialog open={isAddVendorOpen} onOpenChange={setIsAddVendorOpen}>
              ...
              <DialogContent>
                ...
                <form onSubmit={handleVendorSubmit}>
                  <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="vendorName" className="text-right">Name</Label>
                      <Input id="vendorName" name="name" value={newVendor.name} onChange={handleVendorInputChange} className="col-span-3" required />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="vendorEmail" className="text-right">Email</Label>
                      <Input id="vendorEmail" name="email" type="email" value={newVendor.email} onChange={handleVendorInputChange} className="col-span-3" required />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="vendorPhone" className="text-right">Phone</Label>
                      <Input id="vendorPhone" name="phone" type="tel" value={newVendor.phone} onChange={handleVendorInputChange} className="col-span-3" required />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="vendorCompany" className="text-right">Company</Label>
                      <Input id="vendorCompany" name="company" value={newVendor.company} onChange={handleVendorInputChange} className="col-span-3" required />
                    </div>
                  </div>
                  <DialogFooter>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="dashboard">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <Card>
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {vendingMachines.filter(vm => vm.status === 'online').length} online
                    </p>
                  </CardContent>
                </Card>
                <Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">2 low inventory, 1 maintenance</p>
                  </CardContent>
                </Card>
              </div>
              ...
              {selectedMachine === 'all' && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Overview of all tea vending machines</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Table>
                      <TableHeader>
                      ...
                      <TableBody>
                        {vendingMachines.map((machine) => (
                          <TableRow key={machine.id}>
                            ...
                            <TableCell>
                              <Badge variant={
                                machine.replenishmentOrder.status === 'Urgent' ? 'destructive' :
                                machine.replenishmentOrder.status === 'Placed' ? 'default' :
                                machine.replenishmentOrder.status === 'Delivered' ? 'default' :
                                'secondary'
                              }>
                            ...
                            <TableCell>
                              {machine.deliveryBoy ? (
                                <div className="flex items-center">
                                  ...
                                  <span>{machine.deliveryBoy.name} - {machine.deliveryBoy.location}</span>
                                ...
                                <span className="text-muted-foreground">No delivery scheduled</span>
                              )}
                            </TableCell>
              ...
              {selectedMachineData && (
                <Card className="mt-6">
                  <CardHeader>
                    <CardTitle>{selectedMachineData.name} ({selectedMachineData.id})</CardTitle>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <div className="flex items-center space-x-2">
                          <Switch
                            ...
                            onCheckedChange={() => toggleMachineStatus(selectedMachineData.id)}
                          />
                          <span>{selectedMachineData.status === 'online' ? 'Online' : 'Offline'}</span>
                        </div>
                      </div>
                      <div>
                      ...
                      <div className="md:col-span-2">
                        ...
                        <div className="space-y-2">
                          {Object.entries(selectedMachineData.canisterLevels).map(([tea, level]) => (
                      ...
                      <div>
                        <h3 className="text-lg font-semibold mb-2">Replenishment Order</h3>
                        <div className="flex items-center space-x-2">
                          <Badge variant={
                            selectedMachineData.replenishmentOrder.status === 'Urgent' ? 'destructive' :
                            selectedMachineData.replenishmentOrder.status === 'Placed' ? 'default' :
                            selectedMachineData.replenishmentOrder.status === 'Delivered' ? 'default' :
                            'secondary'
                          }>
                      ...
                      <div>
                        ...
                        {selectedMachineData.deliveryBoy ? (
                          <div className="space-y-1">
                            ...
                            <div>Location: {selectedMachineData.deliveryBoy.location}</div>
                        ...
                        <CardContent>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            {Object.entries(categorySales).map(([category, amount]) => (
                              <div key={category}>
                                <p className="text-sm font-medium text-muted-foreground capitalize">{category} Tea Sales</p>
            ...
            <TabsContent value="vendors">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>List of all onboarded vendors and their status</CardDescription>
                </CardHeader>
                <CardContent>
                  <Table>
                    <TableHeader>
                          ...
                          <TableCell>
                            <div className="flex items-center">
                              <Avatar className="h-8 w-8 mr-2">
                                <AvatarImage src={`/placeholder.svg?text=${vendor.name.charAt(0)}`} alt={vendor.name} />
