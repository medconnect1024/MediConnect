...
// Features for the AI Receptionist service
const aiFeatures = [
  "24/7 Availability",
...
const Pricing = () => {
  return (
    <section
      id="pricing"
      className="min-h-[100svh] bg-background w-full flex flex-col gap-5 justify-center items-center px-5 md:px-0 py-10 md:py-20"
    >
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col">
        <div className="text-xl font-semibold">{isFreePlan ? "Free" : "Premium"} Plan</div>
        <h3 className="text-blue-500 text-2xl font-extrabold leading-8">
          {isFreePlan ? "Limited Credits" : "Unlimited Credits"}
        </h3>

        {/* Pricing */}
        <p className="mt-3 flex items-baseline gap-x-1">
          <span className="line-through text-2xl font-sans text-muted-foreground/70">
            {isFreePlan ? "₹100" : "₹1000"}
          </span>
      ...
      {/* Features List */}
      <div className="flex flex-col">
        <ul className="mt-8 space-y-3 text-sm leading-6 text-foreground xl:mt-10">
        ...
        <p className="text-xs text-muted-foreground font-medium mt-5">
          {isFreePlan ? "Limited access to AI receptionist." : "One-time payment. Full access to all features."}
        </p>
      </div>
    </article>
  );
